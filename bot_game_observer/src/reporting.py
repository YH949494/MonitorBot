...
def write_markdown_report(path: Path, summary: SessionSummary) -> None:
    ...
    lines = [
        ...
        f"- **Visual win count:** {summary.visual_win_count}",
        f"- **Any payout count:** {summary.any_payout_count}",
        f"- **Real win count:** {summary.real_win_count}",
        f"- **Break-even count:** {summary.break_even_count}",
        f"- **Net loss with payout count:** {summary.net_loss_with_payout_count}",
        f"- **No payout count:** {summary.no_payout_count}",
        f"- **Result unknown count:** {summary.result_unknown_count}",
        f"- **Visual win rate:** {summary.visual_win_rate:.4f}",
        f"- **Any payout rate:** {summary.any_payout_rate:.4f}",
        f"- **Real win rate:** {summary.real_win_rate:.4f}",
        ...
        f"- **Timeout click→spinning:** {summary.click_to_spinning_timeout_count}",
        f"- **Timeout spinning→result:** {summary.spinning_to_result_timeout_count}",
        f"- **Timeout result→ready:** {summary.result_to_ready_timeout_count}",
        "",
        "## Detector confidence",
        "",
    ]
