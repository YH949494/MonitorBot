...
def generate_missing_synthetic_calibration_screenshots(calibration_root: str | Path, *, overwrite: bool = False) -> list[Path]:
    root = Path(calibration_root)
    generated: list[Path] = []
    for label_path in sorted(root.glob("games/*/labels/*.json")):
        try:
            data = json.loads(label_path.read_text(encoding="utf-8"))
        except Exception:
            continue
        if not _is_synthetic(data.get("notes", "")):
            continue
        image_name = data.get("screenshot")
        if not image_name:
            continue
        image_path = (label_path.parent / image_name).resolve()
        if image_path.exists() and not overwrite:
            continue
